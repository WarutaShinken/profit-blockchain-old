...
async def summary(
    ...
    if amounts is not None:
        print(f"Total Profit farmed: {amounts['farmed_amount'] / units['chia']}")
    ...
    if all_harvesters is not None:
        ...
        for ph, plot_count in PlotStats.staking_addresses.items():
            ...
            print(f"    Balance: {balance} PROFIT")
    ...
    if amounts is None:
        if wallet_not_running:
            print(
                "For details on farmed rewards and fees you should run 'profit start wallet' and 'profit wallet show'"
            )
        elif wallet_not_ready:
            print("For details on farmed rewards and fees you should run 'profit wallet show'")
    else:
        print("Note: log into your key using 'profit wallet show' to see rewards for each key")
