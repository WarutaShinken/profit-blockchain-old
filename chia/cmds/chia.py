...
@click.group(
    help=f"\n  Manage profit blockchain infrastructure ({DIST_NAME} {__version__})\n",
    epilog="Try 'profit start node', 'profit netspace -d 192', or 'profit show -s'",
...
@cli.command("version", short_help="Show profit version")
def version_cmd() -> None:
    print(DIST_NAME, __version__)


@cli.command("run_daemon", short_help="Runs profit daemon")
@click.option(
    "--wait-for-unlock",
    help="If the keyring is passphrase-protected, the daemon will wait for an unlock command before accessing keys",
    default=False,
    is_flag=True,
    hidden=True,  # --wait-for-unlock is only set when launched by profit start <service>
)
@click.pass_context
