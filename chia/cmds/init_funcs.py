...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    if len(all_sks) == 0:
        print("No keys are present in the keychain. Generate them with 'profit keys generate'")
        return None

    config: Dict = load_config(new_root, "config.yaml")
    pool_child_pubkeys = [master_sk_to_pool_sk(sk).get_g1() for sk, _ in all_sks]
    all_targets = []
    stop_searching_for_farmer = "profit_target_address" not in config["farmer"]
    stop_searching_for_pool = "profit_target_address" not in config["pool"]
    ...
    for i in range(number_of_ph_to_search):
        ...
        for sk, _ in all_sks:
            ...
            if all_targets[-1] == config["farmer"].get("profit_target_address"):
                stop_searching_for_farmer = True
            if all_targets[-1] == config["pool"].get("profit_target_address"):
                stop_searching_for_pool = True

    # Set the destinations, if necessary
    updated_target: bool = False
    if "profit_target_address" not in config["farmer"]:
        print(
            f"Setting the profit destination for the farmer reward (1/8 plus fees, solo and pooling) to {all_targets[0]}"
        )
        config["farmer"]["profit_target_address"] = all_targets[0]
        updated_target = True
    elif config["farmer"]["profit_target_address"] not in all_targets:
        print(
            f"WARNING: using a farmer address which we don't have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['farmer']['profit_target_address']} with {all_targets[0]}"
        )

    if "pool" not in config:
        config["pool"] = {}
    if "profit_target_address" not in config["pool"]:
        print(f"Setting the profit destination address for pool reward (7/8 for solo only) to {all_targets[0]}")
        config["pool"]["profit_target_address"] = all_targets[0]
        updated_target = True
    elif config["pool"]["profit_target_address"] not in all_targets:
        print(
            f"WARNING: using a pool address which we don't have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['pool']['profit_target_address']} with {all_targets[0]}"
        )
    if updated_target:
        print(
            f"To change the PROFIT destination addresses, edit the `profit_target_address` entries in"
...
def chia_init(
    ...
    """
    if os.environ.get("PROFIT_ROOT", None) is not None:
        print(
            f"warning, your PROFIT_ROOT is set to {os.environ['PROFIT_ROOT']}. "
            f"Please unset the environment variable and run profit init again\n"
            f"or manually migrate config.yaml"
        )

    print(f"profit directory {root_path}")
    if root_path.is_dir() and Path(root_path / "config" / "config.yaml").exists():
        # This is reached if PROFIT_ROOT is set, or if user has run profit init twice
        # before a new update.
        if testnet:
    ...
    if should_check_keys:
        check_keys(root_path)
    print("")
    print("To see your keys, run 'profit keys show --show-mnemonic-seed'")
