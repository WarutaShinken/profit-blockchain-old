...
service_plotter = "profit plots create"
...
if getattr(sys, "frozen", False):
    name_map = {
        ...
        "profit_farmer": "start_farmer",
        "profit_introducer": "start_introducer",
        "profit_timelord": "start_timelord",
        "profit_timelord_launcher": "timelord_launcher",
        "profit_full_node_simulator": "start_simulator",
    }
...
class WebSocketServer:
    ...
    def _build_plotting_command_args(self, request: Any, ignoreCount: bool, index: int) -> List[str]:
        ...
        command_args: List[str] = ["profit", "plotters", plotter]
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("profit daemon exiting in 5 seconds")
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered PROFIT_ROOT
    os.environ["PROFIT_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with PROFIT_ROOT: {os.environ['PROFIT_ROOT']}")
