...
class BlockTools:
    ...
    async def setup_keys(self):
        ...
        if len(self.pool_pubkeys) == 0 or len(self.farmer_pubkeys) == 0:
            raise RuntimeError("Keys not generated. Run `profit generate keys`")
    ...
    async def new_plot(
        ...
        try:
            ...
            if pool_contract_puzzle_hash is None:
                ...
                pool_address = encode_puzzle_hash(pool_contract_puzzle_hash, "profit")
...
def get_plot_dir() -> Path:
    cache_path = Path(os.path.expanduser(os.getenv("PROFIT_ROOT", "~/.profit/"))) / "test-plots"
    mkdir(cache_path)
    return cache_path
