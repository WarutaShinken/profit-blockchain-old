...
Write-Output "   ---"
Write-Output "Use pyinstaller to create profit .exe's"
Write-Output "   ---"
$SPEC_FILE = (python -c 'import chia; print(chia.PYINSTALLER_SPEC_PATH)') -join "`n"
pyinstaller --log-level INFO $SPEC_FILE

Write-Output "   ---"
Write-Output "Copy profit executables to chia-blockchain-gui\"
Write-Output "   ---"
...
Write-Output "   ---"
Write-Output "Increase the stack for profit command for (profit plots create) chiapos limitations"
...
If ($env:HAS_SECRET) {
   ...
   Write-Output "   ---"
   signtool.exe timestamp /v /t http://timestamp.comodoca.com/ .\release-builds\windows-installer\ProfitSetup-$packageVersion.exe
   signtool.exe verify /v /pa .\release-builds\windows-installer\ProfitSetup-$packageVersion.exe
   }   Else    {
